	         ...
	         */
	         api.ajax({
				...
				if(ret.status){
					...
					var registerstr ='insert into t_self_info(_id,phone_no, token, yjnumber, focusnum, focusednum, nick , sex , birthday , userphoto , slogan) '; 
					registerstr+='values(null,"'+$api.byId('phoneField').value+'","' +ret.token+'"'+',0,0,0,"",null,null,null,null)';	
					
					dbExecuteSql(registerstr,function(ret){
						if (ret) {
