...
function initMap(maph){
	...
	$api.setStorage('photosnum_inrun',0);
	var aMap = api.require('aMap');
	aMap.open({
	    rect: {
	        x: 0,
        ...
        if (ret) {  
		    ...
		    ajaxRequest(getdayviewUrl + JSON.stringify(yj_urlParam), 'GET', '', function (ret, err) {   
		        ...
		        if (ret && ret[0]!=null && ret[0].dayview!=null && ret[0].dayview[0]!=null) {  	        	
		        	  ...
		        	  //showdayviewpoint(dayviewid);		        	   					       
					shownewpoint(dayviewid,file,imgurl);      
					...
					var setdayviewbodyParam = {
				        dayinfo:newday,
				    }		   
					 ajaxRequest(setdayviewUlr, 'post', JSON.stringify(setdayviewbodyParam),function (ret, err) {
					 	//alert(JSON.stringify(ret));
					 	if(ret){
	 ...
	 ajaxRequest(recordsUlr+JSON.stringify(recordsUlr_Param), 'GET', '', function (ret, err) {
        if (ret) {
            ...
            for(var id in t){	
        		...
        		//$api.setStorage('photosnum_inrun',$api.getStorage('photosnum_inrun')*1.0+1);
            	i++;	            	            	
 ...
 *显示新添加纪录点 
 */
function shownewpoint(dayviewid,file,imgurl){
	
	getlocation(function(recpoint){		
		//alert(JSON.stringify(recpoint));
		var currecpoint= recpoint;
	...
	button.open({
	    ...
	    if (ret) {	 
	    	...
	    	if(ret.eventType=="click")
	    	{
	    		...
	    		if($api.getStorage('photosnum_inrun')==0){
	    			api.confirm({
					    title: '温馨提示',
					    msg: '还没有记录,结束旅程还是继续记录？',
					    buttons: ['结束', '继续']
						}, function(ret, err) {
					    var index = ret.buttonIndex;
					     if(index==1){
					     	$api.rmStorage('intravel');
				     	 	//$api.rmStorage('travelId');
					    	api.closeToWin({
				            name: 'slide'
					        });
					     }
