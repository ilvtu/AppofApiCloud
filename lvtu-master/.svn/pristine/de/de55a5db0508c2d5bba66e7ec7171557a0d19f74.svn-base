	...
	if(length==choosephotos.length){
		 ...
		 */
	        ...
	        imageFilter.compress({
	                ...
	                save : {
	                        album : true,
	                        imgPath : imageCachePath+'/lvtutmp/',
	                        imgName : img_Name + '.jpg'
	                }
	        }, function(reta, err) {
	                if (reta.status) {
	                        var compressedImgPath = imageCachePath + '/lvtutmp/' + img_Name + '.jpg';
...
function photoFilter(picList) {
    ...
    //alert(JSON.stringify(g_data));
    if(g_data!=null && g_data[0]!=null && g_data[0].path!=null)
    {
    	api.showProgress({
	        style: 'default',
	        animationType: 'fade',
	        title: '先喝杯茶...',
	        text: '共有照片'+g_data.length+'张',
	        modal: false
	    });  
...
function checkDateRange2(start, end, current){
    ...
    var curDate;
    current.replace(reg, function fit($1){
        curDate = new Date(Date.parse($1.replace(/-/g,"/")));
    });
